...
@RunWith(Suite.class)
@Suite.SuiteClasses({
    UnchainShortestIteratorTest.Functions.class,
    UnchainShortestIteratorTest.Degenerations.class
})
public class UnchainShortestIteratorTest {
    ...
    public static class Functions {
        ...
        public void canVerifyIfAnIteratorHasNext() {
            ...
            Iterator<List<Integer>> iter = new UnchainShortestIterator<List<Integer>, Integer>(SIZE_IS_ALWAYS_1, backingList.iterator(), LIST_FACTORY);
        ...
        public void callingHasNextOnTooShortIteratorYieldsFalse() {
            List<Integer> backingList = Arrays.asList(1);
            Iterator<List<Integer>> iter = new UnchainShortestIterator<List<Integer>, Integer>(SIZE_IS_ALWAYS_2, backingList.iterator(), LIST_FACTORY);
    ...
    public static class Degenerations {
        ...
        public void creatingWithNullIteratorYieldsException() {
            new UnchainShortestIterator<List<Integer>, Integer>(SIZE_IS_ALWAYS_1, null, LIST_FACTORY);
        }

        @Test(expected = IllegalArgumentException.class)
        public void creatingWithNullChannelProviderYieldsException() {
            new UnchainShortestIterator<List<Integer>, Integer>(null, Collections.<Integer>emptyList().iterator(), LIST_FACTORY);
        }

        @Test(expected = IllegalArgumentException.class)
        public void creatingWithNullListFactoryYieldsException() {
            new UnchainShortestIterator<List<Integer>, Integer>(SIZE_IS_ALWAYS_2, Collections.<Integer>emptyList().iterator(), null);
        }

        @Test(expected = NoSuchElementException.class)
        public void consumingNonSquaredIteratoryYieldsException() {
            Iterator<List<Integer>> iter = new UnchainShortestIterator<List<Integer>, Integer>(SIZE_IS_ALWAYS_2, Arrays.asList(1, 2, 2).iterator(), LIST_FACTORY);
            iter.next();
            iter.hasNext();
            iter.next();
        }
